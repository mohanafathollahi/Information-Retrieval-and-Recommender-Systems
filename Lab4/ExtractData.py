...
if __name__ == '__main__':
    ...
    parser.add_argument('--freqs', default=None, type=str, required=False, help='Used Frequencies for the experiment')
    ...
    try:
        ...
        if freqs is not None:
            print('Saving data ...')
            vocabulary_output = "vocabulary-"+freqs+".txt"
            f = open(vocabulary_output, 'w')
            for p in sorted(lwords):
                f.write(p.encode('ascii', 'replace').decode() + ' ' + str(voc[p]) + '\n')
            f.flush()
            f.close()

            documents_output = "documents-"+freqs+".txt"
            f = open(documents_output, 'w')
            for doc in docterms:
                docname = doc.split('/')
                ...
                if docvec:  # writes the document if there are words from the vocabulary
                    docvec = [x for x in docvec.split(" ") if x]
                    f.write(str(len(docvec)) + '\n')
            f.flush()
            f.close()
        else:
            print('Saving data ...')
