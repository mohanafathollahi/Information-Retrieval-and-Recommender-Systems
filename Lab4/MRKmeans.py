...
if __name__ == '__main__':
    ...
    for i in range(args.iter):
        ...
        with mr_job1.make_runner() as runner1:
            ...
            for cluster, (prototype, documents) in mr_job1.parse_output(runner1.cat_output()):
                new_assign[cluster] = documents
                new_proto[cluster] = prototype

            # If your scripts returns the new assignments you could write them in a file here
            with open(cwd+"/"+f"assignments{i}.txt", "w") as file:
                for cluster, documents in new_assign.items():
                    file.write(f'{cluster}: {" ".join(documents)}\n')

            # You should store the new prototypes here for the next iteration
            with open(cwd+"/"+f"prototypes{i+1}.txt", "w") as file:
                for cluster, prototype in new_proto.items():
                    word_frequency_list = [f"{word}+{frequency}" for word, frequency in prototype]
                    file.write(f'{cluster}:{" ".join(word_frequency_list)}\n')
